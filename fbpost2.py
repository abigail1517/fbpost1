...
RUN_ID = f"{REPO_ID}:{os.environ.get('GITHUB_RUN_ID') or uuid.uuid4().hex[:8]}"
# RUN_ID identifies THIS SPECIFIC job execution and is what actually gets
# written to LockedBy/LockedAt (the transient "someone has a live browser
# open on this page right now" lock). REPO_ID (stable across runs) is only
# used for the PERSISTENT AssignedRepo ownership column. Deliberately NOT
# using REPO_ID for the live lock: if two job runs of the SAME repo are
# ever active at once (e.g. a new run started slightly before the old one
# finished, to avoid a session gap), they'd otherwise both see "locked by
# my own repo" and treat the page as free, and both open a browser on the
# same Facebook page at the same time. With RUN_ID, a live second run
# correctly sees the page as locked by a DIFFERENT run and waits/skips it
# until the first run's heartbeat stops and the lock naturally expires (or
# the first run exits cleanly and releases it) — see LockTtlMinutes.
...
SETTINGS_DEFAULTS = {
    ...
    "urlreplaceenabled":      "TRUE",
    "postmode":               "rotation",
    "lockttlminutes":         "25",     # a lock with no heartbeat for this long is considered
                                       # abandoned (crashed/killed job) and can be reclaimed.
                                       # Keep comfortably above HeartbeatMinutes so a healthy
                                       # run's own lock never expires out from under it.
    "heartbeatminutes":       "15",
...
def claim_pages(sh: Sheet, master: dict) -> list[dict]:
    ...
    for row in active_rows():
        ...
        # Uses RUN_ID (unique per job), NOT REPO_ID, and there is NO
        # identity-bypass here on purpose: if this repo's PREVIOUS run is
        # still actively heartbeating this page (e.g. you deliberately
        # started the next scheduled job a few minutes before the old one
        # finished, to avoid a session gap), it must be treated as
        # genuinely locked — not "oh that's my repo, free to take." Only a
        # heartbeat going stale for longer than LockTtlMinutes (i.e. that
        # run actually died) makes the page reclaimable.
        locked_by = row.get("LockedBy", "").strip()
        locked_at = row.get("LockedAt", "").strip()
        is_free = (not locked_by) or (minutes_since(locked_at) > ttl)
        ...
        sh.write_cell(PAGES_TAB, row["_row"], lb_col, RUN_ID)
        ...
        if fresh_row and fresh_row.get("LockedBy", "").strip() == RUN_ID:
            ok(None, f"[{page_id}] claimed by '{RUN_ID}'")
...
def heartbeat_lock(sh: Sheet, page_row_num: int, lb_col: int, la_col: int):
    sh.write_cell(PAGES_TAB, page_row_num, lb_col, RUN_ID)
...
class PageWorker:
    ...
    async def _run_locked(self):
        ...
        async with async_playwright() as p:
            ...
            try:
                while True:
                    await self._refresh_cfg()  # pick up ANY sheet edits made since last cycle
                    ...
                    if elapsed_min >= self.cfg.max_runtime_minutes:
                        info(cfg.page_id, f"Runtime window ({self.cfg.max_runtime_minutes}m) reached — stopping")
                        break

                    # Sleep until the next post, heartbeating (and staying
                    # "live") every heartbeat_minutes in between.
                    remaining = self.cfg.loop_interval_minutes * 60
                    while remaining > 0:
                        chunk = min(remaining, self.cfg.heartbeat_minutes * 60)
                        await asyncio.sleep(chunk)
                        remaining -= chunk
                        if (time.monotonic() - last_heartbeat) / 60 >= self.cfg.heartbeat_minutes:
                            await self._heartbeat()
                            last_heartbeat = time.monotonic()
                        if (time.monotonic() - start) / 60 >= self.cfg.max_runtime_minutes:
                            remaining = 0
            finally:
                await self._release()
                await self.browser.close()
    ...
    async def _refresh_cfg(self):
        """Re-reads the Settings tab and this page's own Pages row fresh
        from the sheet, and rebuilds self.cfg from them. Called at the top
        of every post cycle so that edits made WHILE the job is running —
        a new Caption, a toggled UrlReplaceEnabled, a changed
        LoopIntervalMinutes, a new MegaFolder, anything — take effect on
        the very next cycle, instead of only being picked up the next time
        the whole workflow job restarts. max_runtime_minutes is
        deliberately NOT refreshed here (it's tied to this job's own
        elapsed-time budget, decided once at job start)."""
        try:
            master = load_master_settings(self.sh)
            rows, _ = self.sh.as_dicts(PAGES_TAB)
            row = next((r for r in rows if r.get("PageId", "").strip() == self.cfg.page_id), None)
            if row is None:
                warn(self.cfg.page_id, "Page row no longer found in sheet on refresh — "
                                        "keeping the last known settings for this cycle")
                return
            self.cfg = build_page_config(row, master, self.cfg.max_runtime_minutes)
            self.col_index = get_pages_col_index(self.sh)
        except Exception as e:
            warn(self.cfg.page_id, f"Could not refresh settings from sheet this cycle "
                                    f"(keeping last known values): {e}")
...
async def main_async(once: bool):
    ...
    info(None, f"Runner '{REPO_ID}' (run '{RUN_ID}') — {len(configs)} page(s) claimed, "
...
def main():
    ...
    print(f"🚀 Runner '{REPO_ID}' starting run '{RUN_ID}' — "
