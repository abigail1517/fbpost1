"""
fbpost.py — MULTI-PAGE / MULTI-THREADED VERSION (simplified locking)
──────────────────────────────────────────────────────────────
...
at the same time — pages are ASSIGNED (pinned) to a runner in the sheet,
so a second repo running concurrently only ever picks up pages nobody
else currently owns.
...
Run modes:
    ...
    python -u fbpost.py                   # long-running: loops every page on its
                                            # own interval until MaxRuntimeMinutes

Identify this runner (used for AssignedRepo) with the REPO_ID env var, or
it defaults to GITHUB_REPOSITORY (stable across runs of the same repo).
...
Tab "Settings" — MASTER defaults, two columns, one row per setting:
    ...
    AssignedTtlMinutes      1440       # how long an AssignedStatus=InUse page can go
                                       # without a heartbeat before another repo may take it
...
FbStorageState is an OVERRIDE: leave it blank to fall back to Settings.
    ...
    AssignedRepo         written by the script — which runner "owns" this page (sticky)
    AssignedStatus        written by the script — "InUse" while a runner is actively
                          working this page, "Idle" once it's free again (still owned
                          by AssignedRepo, so that SAME repo reclaims it instantly next run)
...
SETTINGS_DEFAULTS = {
    ...
    "assignedttlminutes":     "1440",  # 24h — how long an AssignedStatus=InUse page may go
                                       # without a heartbeat before another repo can take it
...
def claim_pages(sh: Sheet, master: dict) -> list[dict]:
    ...
    Single-layer ownership, entirely via AssignedRepo / AssignedStatus / AssignedAt:
      ...
      - AssignedRepo   : which runner owns this page (sticky — stays even when idle)
      - AssignedStatus : "InUse" while a runner is actively working the page,
                         "Idle" once it's free again
      - AssignedAt     : last claim/heartbeat/release timestamp

    A page belonging to a DIFFERENT repo is only ever taken over if its
    AssignedStatus isn't "InUse" AND AssignedAt has gone stale past
    AssignedTtlMinutes (meaning that repo's workflow is disabled/broken).
    Otherwise we skip it outright — no separate lock column needed.
    """
...
class PageWorker:
    ...
    async def _release(self):
        ...
        if as_col is not None and aa_col is not None:
            # AssignedRepo is left untouched (sticky) — this repo reclaims
